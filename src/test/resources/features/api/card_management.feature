...
Feature: Card Management API
  ...
  Scenario: Block a newly issued card
    When I issue a "DEBIT" card for account "ACC-001"
    And I block the last issued card
  ...
  Scenario: Unblock a blocked card
    When I issue a "DEBIT" card for account "ACC-001"
    And I block the last issued card
    And I unblock the last issued card
    Then the response status should be 200
    And the card status should be "ACTIVE"

  @pin @regression
  Scenario: Validate a correct PIN format
    When I issue a "DEBIT" card for account "ACC-001"
    And I validate PIN "1234" for the last issued card
    Then the response status should be 200
  ...
  Scenario: Validate an incorrect PIN format
    When I issue a "DEBIT" card for account "ACC-001"
    And I validate PIN "ab" for the last issued card
    Then the response status should be 200
